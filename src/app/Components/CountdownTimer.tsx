...
export default function CountdownTimer(
    ...
    function calculateTimeLeft() : CountdownTimeLeft{
        ...
        let diffence = deadline.getTime() - currentDate.getTime();
  ...
  return (
    ...
    <div className=" text-xl flex flex-col gap-4 w-fit px-2 py-2">
        ...
        <h3
        className="!text-xl text-(--text-invert)/90 ">
            <span
            className="text-(--yellow) text-xs block">
        {category}</span>
            {title}
        </h3>
        ...
        <div className="flex gap-2">
            ...
            className=" border-1 border-(--yellow)/20 rounded-md text-center max-w-[52px] py-1 w-[46px] px-2">
                ...
                <p
                className="text-sm text-(--text-invert)/80 font-semibold">
                    {unit}
                </p>
            </div>
            ))}
